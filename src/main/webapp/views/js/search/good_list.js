require(['common/util', 'component/iframeLayer', 'common/http', 'handlebars', 
	...
	function ztreeInit(){
		//加载权限资源
		http.httpRequest({
			url:'/zzLH/goodClassInfo/showGoodClassTree',
			type:'post',
	...
	function showPermision(id,parentNode){
		...
		http.httpRequest({
			type:"post",
			url:"/zzLH/goodClassInfo/selectByUid",
			data:{uid:id},
			success:function(data){
				if(data.status=='success'){
		...
		util.bindEvents([{
			...
			handler:function(){
				...
				layer.dialog({
					title:'新增商品类别',
					area:['580px', '300px'],
					content:'/zzLH/goodClassInfo/showAddAndEdit?sid='+sid,
			...
			handler:function(){
				...
				layer.confirm("确定要删除该商品类别吗？",{icon:2,title:'提示'},function(index){
					http.httpRequest({
						type:'post',
